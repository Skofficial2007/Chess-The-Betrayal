...
namespace ChessTheBetrayal.Tests.EditMode.Core.Engine.Movement
{
    ...
    public class BishopMovementTests
    {
        ...
        public void GetLegalMoves_BishopOnEmptyBoard_ReturnsThirteenDiagonalMoves()
        {
            ...
            Vector2Int d4 = BoardSetup.AlgebraicToVector("d4");
        ...
        public void GetLegalMoves_BishopBlockedByFriendlyPieceOnDiagonal_CannotPassThrough()
        {
            ...
            // Act
            ChessEngine.GetLegalMoves(board, BoardSetup.AlgebraicToVector("c1"), _outputBuffer);

            // Assert
            HashSet<Vector2Int> dests = BoardSetup.GetDestinations(_outputBuffer);

            Assert.That(dests.Contains(BoardSetup.AlgebraicToVector("d2")), Is.True);
            Assert.That(dests.Contains(BoardSetup.AlgebraicToVector("e3")), Is.True);
            Assert.That(dests.Contains(BoardSetup.AlgebraicToVector("f4")), Is.False, "Cannot capture friendly.");
            Assert.That(dests.Contains(BoardSetup.AlgebraicToVector("g5")), Is.False, "Cannot pass through.");
            Assert.That(dests.Contains(BoardSetup.AlgebraicToVector("h6")), Is.False);
        }
        ...
        public void GetLegalMoves_BishopColorBound_NeverLandsOnOppositeColor()
        {
            ...
            // Assert
            Vector2Int c1 = BoardSetup.AlgebraicToVector("c1");
