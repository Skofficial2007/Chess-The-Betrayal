...
namespace ChessTheBetrayal.Tests.EditMode.Core.Engine.Betrayal
{
    ...
    public class BetrayalRetributionTests
    {
        ...
        public void GetRetributionMoves_KingCanActAsExecutioner_IncludedInResults()
        {
            ...
            // Act
            ChessEngine.GetRetributionMoves(board, Team.White, BoardSetup.AlgebraicToVector("e2"), _moveBuffer);
        ...
        public void GetRetributionMoves_OnlyCapableExecutionerIsPinned_ReturnsEmpty()
        {
            ...
            // Act
            ChessEngine.GetRetributionMoves(board, Team.White, BoardSetup.AlgebraicToVector("d4"), _moveBuffer);
        ...
        public void GetRetributionMoves_MultipleCapableExecutioners_AllNonPinnedOnesIncluded()
        {
            ...
            Assert.That(_moveBuffer[0].StartPosition, Is.EqualTo(BoardSetup.AlgebraicToVector("a4")), "Only the unpinned Executioner should be returned.");
        }
        ...
        public void GetRetributionMoves_TargetMustBeBetrayerSquareExactly_OtherCapturesExcluded()
        {
            ...
            // Act
            ChessEngine.GetRetributionMoves(board, Team.White, BoardSetup.AlgebraicToVector("h4"), _moveBuffer);

            // Assert
            Assert.That(_moveBuffer.Count, Is.EqualTo(1));
            Assert.That(_moveBuffer[0].EndPosition, Is.EqualTo(BoardSetup.AlgebraicToVector("h4")), "Executioner must only target the Betrayer square.");
        }
    }
}